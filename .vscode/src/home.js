...
function showSessionExpiredModal() {
    ...
    if (!modal) return;
    // ativa a classe que mostra o modal (CSS usa .session-modal.active)
...
function fecharModal() {
    ...
    // limpar sessão local e redirecionar para a tela de login/visitante
...
function scheduleSessionTimeout() {
    ...
    if (!expires) return; // nada a fazer sem cookie
    ...
    if (remaining <= 0) {
        // expirou — mostrar modal imediatamente
        showSessionExpiredModal();
    } else {
        // agendar popup para quando expirar
        setTimeout(() => {
            showSessionExpiredModal();
        }, remaining + 250); // pequeno buffer
    }
}

// também mostrar modal caso backend responda 401 em qualquer fetch
...
// iniciar agendamento ao carregar a página
document.addEventListener('DOMContentLoaded', () => {
